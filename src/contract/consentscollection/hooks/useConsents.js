...
export const useConsents = (contractId) => {
    ...
    const [zudConsent, setZudConsent] = useState(null);
    ...
    const fetchConsents = async () => {
        try {
            ...
            setZudConsent(response.zudConsent);
            return response;
        } catch (error) {
    ...
    const updateZudConsent = async (consentData) => {
        try {
            await fetchData(
                `contracts/consents/${contractId}/zud-consent`,
                'PUT',
                consentData
            );
            await fetchConsents();
        } catch (error) {
            console.error('Error updating ZUD consent:', error);
            throw error;
        }
    };

    const markZudConsentAsSentByMail = async () => {
        try {
            await fetchData(
                `contracts/consents/${contractId}/zud-consent/sent-by-mail`,
                'POST'
            );
            await fetchConsents();
        } catch (error) {
            console.error('Error marking ZUD consent as sent by mail:', error);
            throw error;
        }
    };

    const addZudConsentAgreement = async (file) => {
        try {
            const formData = new FormData();
            formData.append('file', file);
            
            await fetchData(
                `contracts/consents/${contractId}/zud-consent/agreement`,
                'POST',
                formData,
                {
                    headers: {
            ...
            console.error('Error uploading ZUD consent agreement:', error);
            throw error;
        }
    };

    const invalidateZudConsent = async (reason) => {
        try {
            await fetchData(
                `contracts/consents/${contractId}/zud-consent/invalidate`,
                'POST',
                { reason }
            );
            await fetchConsents();
        } catch (error) {
            console.error('Error invalidating ZUD consent:', error);
            throw error;
        }
    };
